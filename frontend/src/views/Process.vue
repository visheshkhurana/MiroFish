<template>
  <div class="process-page">
    <!--  -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">PREDICT</div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">GraphDataLoading...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">GenerateCompleteAutoStartBuildGraph</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">Build</span>
        </div>
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  Upload，LLMAnalysisContent，AutoGenerateSimulation（EntityType + RelationshipType）
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">GenerateEntityType ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">GenerateRelationshipType ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} Relationship...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label"></div>
                <div class="detail-content">
                  Generate， Zep API BuildKnowledge Graph，EntityRelationship
                </div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  <div class="result-item">
                    ...
                    <span class="result-label">EntityNode</span>
                  </div>
                  <div class="result-item">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">EntityType</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">BuildComplete</div>
                <div class="phase-api">EnterNext Step</div>
              </div>
...
const currentPhase = ref(-1) // -1: Upload, 0: Generate, 1: Graph Building, 2: Complete
...
const statusText = computed(() => {
  ...
  if (currentPhase.value >= 2) return 'BuildComplete'
...
// ProcessProject -  ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'UploadFile，BackOperation'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Generate
    ontologyProgress.value = { message: 'UploadFileAnalysis...' }
    ...
    error.value = 'ProjectInitializeFailed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = 'LoadProjectFailed: ' + (err.message || 'UnknownError')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Graph Building...'
      
      // Save task_id rounds
      const taskId = response.data.task_id
      
      // GraphDatarounds（Statusrounds）
      startGraphPolling()
      
      // Statusrounds
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'Graph BuildingFailed'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Graph BuildingFailed: ' + (err.message || 'UnknownError')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // FetchProjectInfoFetch graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Graph BuildingComplete，LoadData...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // UpdateProgressCompleteStatus
        buildProgress.value = {
          progress: 100,
          message: 'BuildComplete，LoadGraph...'
        }
        
        // LoadProjectDataFetch graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 LoadGraph:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph BuildingFailed: ' + (task.error || 'UnknownError')
        buildProgress.value = null
      }
    }
  } catch (err) {
