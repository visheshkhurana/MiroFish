  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        ...
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="Maximum/">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAGReal-timeUpdateMedium' : 'Real-timeUpdateMedium...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">ContentProcessing，RefreshGraph</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="CloseTip">
...
const showEdgeLabels = ref(true) // DefaultShowEdgeLabel
const expandedSelfLoops = ref(new Set()) // Expand 
const showSimulationFinishedHint = ref(false) //  Tip
const wasSimulating = ref(false) // YesNoinMedium
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // MediumisStatus，ShowTip
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Expand/CollapseStatus
...
const renderGraph = () => {
  ...
  // ProcessEdgeData，Node EdgeQuantityand
  const edgePairCount = {}
  const selfLoopEdges = {} // Node Edge
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // StatisticsNode EdgeQuantity，Edge
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Edge Info
        }
      })
      return
    }
  ...
  // Simulation - Based onEdgeQuantityNode
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Based onthisNode EdgeQuantity
      //  150，Edge 40
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // ResetMediumEdge Style
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // HighCurrentMedium Edge
