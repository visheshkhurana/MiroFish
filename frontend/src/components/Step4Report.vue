    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - inCompleteShow -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (ContentComplete， AcceptableComplete) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    // AnalysisQuestion
    const queryMatch = text.match(/AnalysisQuestion:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Prediction
    const reqMatch = text.match(/Prediction:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // StatisticsData - "Prediction: X"
    const factMatch = text.match(/Prediction:\s*(\d+)/)
    const entityMatch = text.match(/:\s*(\d+)/)
    const relMatch = text.match(/Relationship:\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    // Question - ，Quantity
    const subQSection = text.match(/### Analysis Question\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    const relSection = text.match(/### 【Relationship】\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parseInterview = (text) => {
  ...
  try {
    // Theme
    const topicMatch = text.match(/\*\*Theme:\*\*\s*(.+?)(?:\n|$)/)
    ...
    // Select
    const reasonMatch = text.match(/### Select\n([\s\S]*?)(?=\n---\n|\n### )/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^not|^|^Select/)) {
          //  （）
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          //  Reddit ，inCopyisDefaultShow
          if (interview.redditAnswer && interview.redditAnswer !== '（notReply）') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '（notReply）') {
    ...
    // Summary
    const summaryMatch = text.match(/### SummaryandViewpoint\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    // Search
    const queryMatch = text.match(/Search:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Prediction: '),
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Relationship (${props.result.relations.length})`)
        ]),
        props.result.subQueries.length > 0 && h('button', {
          ...
          h('span', { class: 'tab-label' }, `Question (${props.result.subQueries.length})`)
        ])
      ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Medium Latest'),
          ...
          }, expandedFacts.value ? ` ▲` : `ExpandAll ${props.result.facts.length}  ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? ` ▲` : `ExpandAll ${props.result.entities.length}   ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? ` ▲` : `ExpandAll ${props.result.relations.length}  ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'AnalysisQuestion'),
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'NoneCurrent'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'None'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'NoneRelationship')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? ` ▲` : `ExpandAll ${props.result.historicalFacts.length}  ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // is Question-Maintain SelectStatus
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（notReply）' || t === '(notReply)' || t === '[Reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Support：
      // 1. "QuestionX：" or "QuestionX:" （Medium，Backend）
      // 2. "1. " or "\n1. " （+，）
      let matches = []
      let match
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'SearchResults'),
            h('span', { class: 'panel-count' }, ` ${props.result.facts.length} `)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'notResults'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? ` ▲` : `ExpandAll ${props.result.facts.length}  ▼`)
...
const renderMarkdown = (content) => {
  ...
  //  Title（## xxx），BecauseTitlealreadyinShow
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Clear loading Status
            emit('update-status', 'completed')
            stopPolling()
            // in  nextTick MediumProcess
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // IfTaskalreadyComplete，；OtherwiseLatestLog
            if (isComplete.value) {
...
// AnswerContent -  LLM response MediumContent
const extractFinalContent = (response) => {
  if (!response) return null
  
  //  <final_answer> Label Content
  ...
  //  Answer:  Content
  const chineseFinalMatch = response.match(/Answer[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If ## or # or > ，AcceptableYes  markdown Content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // IfContentmarkdown，RemoveBack
...
/* Console Logs - and Step3Simulation.vue  */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
